...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:frosty\n' 
            + 'ORG:Creador del bot-cabezon\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5491157408121:+54 9 11 5740-8121\n' 
            + 'END:VCARD'
//WhatsApp Conexión
const
	{
...
async function starts() {
	...
	console.log(color('[ BOT]','aqua'), color("BOT DE FROSTY", "yellow"))
	console.log('>', '[',color('INFO','blue'),']','BOT DE FROSTY')
	FxBot.on('qr', () => {
...
┗━━━━━━━━━━━━━━━━━`
				...
				try {
					...
					ppimg = 'https://media.tycsports.com/files/2021/09/10/330304/leo-messi_416x234.jpg'
				}
				teks = `*@${num.split('@')[0]}*
${leave}	`
				let buff = await getBuffer(ppimg)
			...
			try {
					...
					ppimg = 'https://s.france24.com/media/display/9423a814-f832-11eb-adc8-005056a90284/w:1280/p:16x9/messi%20bye.jpg'
				}
			let buff = await getBuffer(ppimg)
...
${promote}`
			...
			try {
					...
					ppimg = 'https://e00-us-marca.uecdn.es/claro/assets/multimedia/imagenes/2021/08/05/16281955252788.jpg'
				}
			let buff = await getBuffer(ppimg)
			teks = `matamos a un admin por pito chico
...
//SETTING
       ...
       const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
		...
		mess = {
			...
			wrongFormat: '「 ❗ 」 No seas pelotudo escribi bien el formato',
			waitmusic: '「 ❗ 」 Esperaaaa gato estoy buscando tu musica\nEn cuánto la encuentre la envío!!',
			waitimg: '「 ❗ 」 Esperaaaa gato estoy creando tu imagen!',
			nsfwoff: '「 ❗ 」La Funcion De Nsfw No Está Activa!',
					musica: '「 ❗ 」Espera un toke amigo, estoy buscando tu canción',
			imageoff: '「 ❗ 」No Puedo Enviar Fotos Mientras No Esta Activa La Función!',
			error: {
				stick: '「 ❗ 」 No se pudo convertir, algo hiciste mal',
				errostick: '❌Error al crear el sticker❌',
				Iv: '「 ❗ 」Link Invalido como vos ajsjdasjd'
			},
			only: {
				group: '「 ❗ 」Este Comando Solo Puede Ser Usado En Grupos',
				admin: '「 ❗ 」No sos admin negro',
				Badmin: '「 ❗ 」 Dame admin y te ayudo'
			}
		}
		const isUrl = (url) => {
...
if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(`「❗」Si me das admin podes usarlo`)
					...
					mentions(`Chau pedazo de gordo novix de fajo @${mentioned[0].split('@')[0]} 🥵`, mentioned, true)
					FxBot.groupRemove(from, mentioned)
					}					
					break 
					case prefix+ 'promote':
                 ...
                 case prefix+ 'pm':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Recibido mirey✅\n\nAgregando como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						FxBot.groupMakeAdmin(from, mentioned)
					}
					break
...
case prefix+ 'leave':
					...
					FxBot.groupLeave (from) 
                     ...
                     setTimeout( () => {
					FxBot.updatePresence(from, Presence.composing) 
					FxBot.sendMessage(from, 'Adios inutiles👋', text) // ur cods
					}, 0)
                     break
...
case prefix+ 'tagall2':
				...
				FxBot.updatePresence(from, Presence.composing) 
					...
					FxBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'✪ FROSTY ┃ BOT ✪', text, {quoted: mek})
					break
                case prefix+ 'tagall3':
                case prefix+ 'marcar3':
				FxBot.updatePresence(from, Presence.composing) 
...
case prefix+ 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Para activar está funcion pone *welcome 1')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ya esta activada inutil')
						welkom.push(from)
...
case prefix+ 'tovid':
    ...
    case prefix+ 'toimg':
			...
			fakethumb(buffer,'BOT DE FROSTY')
			fs.unlinkSync(ran)
			})
			break
case prefix+ 'tourl':
...
case prefix+ 'stickergif': 
    ...
    case prefix+ 's':
            ...
            fs.unlinkSync(ran)
                ...
                reply(`Envía una imagen con una leyenda ${prefix}sticker\nDuracion Sticker Video 1-9`)
            }
            break

case prefix+ 'attp':
						if (args.length < 1) return reply(`Y El Texto??\n> *Ejemplo* : *${prefix}attp* pito chico`)
						 var itsme = `0@s.whatsapp.net`
			   	 var split = `Texto De Colores`
		     	   var selepbot =         {
...
susp = `🚫@${mentioned[0].split('@')[0]} no puede usarme más por gay🚫`
...
fs.writeFileSync('./database/banned.json', JSON.stringify(ban))
susp = `❎@${mentioned[0].split('@')[0]} ya puede usarme de nuevo❎`
mentions(`${susp}`, mentioned, true)   
break
...
.then((a) => {
if (Number(filesize) >= 30000) return sendMediaURL(from, thumb, `*DESCARGADOR DE AUDIO BY FROSTY*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*:MP3\n*⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración de más del límite se presenta en forma de enlace_`)
const captions = `*DESCARGADOR DE AUDIO BY FROSTY*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*:MP3\n*⚖️Tamaño*:${filesizeF}\n\n_ESPERA QUE ESTÁ ENVIANDO EL ARCHIVO, NO SPAMES LA CONCHA DE TU MADRE_`
...
reply(mess.mpv)
				...
				axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
				.then((a) => {
				if (Number(filesize) >= 40000) return sendMediaURL(from, thumb, `*DESCARGADOR DE VIDEO BY FROSTY*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*:MP3\n*File⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración de más del límite se presenta en forma de enlace_`)
	...
	case prefix+ 'ytsearch':
			if (args.length < 1) return reply('Que queres buscar pedazo de gil?')
			var srch = args.join('');
			try {
...
case prefix+ 'play':
   			...
   			var mulaikah = aramat[0].url							
                  try {
                    ...
                    .then((res) => {
                        ...
                        axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                        .then(async (a) => {
                        if (Number(filesize) >= 100000) return sendMediaURL(from, thumb, `*DESCARGADOR DE MUSICA BY FROSTY*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*: MP3\n*⚖️Tamaño*: ${filesizeF}\n*📎Link* : ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES LA CONCHA DE TU MADRE_`)
		                        const captions = `*DESCARGADOR DE MUSICA BY FROSTY*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*: MP3\n*⚖️Tamaño*: ${filesizeF}\n*📎Link*: ${a.data}\n\n_ESPERA QUE ESTÁ ENVIANDO EL ARCHIVO, NO SPAMES LA CONCHA DE TU MADRE_`
                        sendMediaURL(from, thumb, captions)
    ...
    FxBot.sendMessage(from, options, text, { quoted: mek } )
				break
case prefix+ 'creador':
await FxBot.sendMessage(from, {displayname: "FROSTY", vcard: vcard}, MessageType.contact)
break
